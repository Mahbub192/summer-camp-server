...
async function run() {
  try {
    ...
    const selectedClassesCollection = client
      .db("summerDB")
      .collection("selected");
    const pendingClassCollection = client.db("summerDB").collection("pending");
...
app.get("/", (req, res) => {
  res.send("Summer camp are coming");
});
app.listen(port, () => {
  console.log("Summer camp server running port number ", port);
});
